...
let numFavoritesSounds = {}
for (const user of users) {
  for (const key in user.favoritesSounds) {
    if (numFavoritesSounds[key] !== undefined) {
      numFavoritesSounds[key] = numFavoritesSounds[key] + 1
    } else {
      numFavoritesSounds[key] = 1
    }
  }
}

console.log(numFavoritesSounds)
